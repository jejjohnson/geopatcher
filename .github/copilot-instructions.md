...
- **Layout**: `src/` layout (`src/geopatcher/`)
- **Testing**: pytest
...
**All four checks must pass before any commit.** CI runs them on the entire repo (`ruff check .`), not just `src/geopatcher/`, so always run the commands below from the repo root.
...
uv run --group typecheck ty check src/geopatcher
```

> **Common pitfall**: Running `ruff check src/geopatcher/` instead of `ruff check .` misses import-sorting errors in `tests/` and `scripts/`. The CI workflow runs `ruff check .`. Always use `.` (repo root), not a subdirectory.
